...
describe 'vpm unpublish', ->
  ...
  beforeEach ->
    ...
    process.env.VIA_HOME = temp.mkdirSync('vpm-home-dir-')
  ...
  describe "when no version is specified", ->
    it 'unpublishes the package', ->
      ...
      vpm.run(['unpublish', '--force', 'test-package'], callback)
    ...
    describe "when --force is not specified", ->
      it 'prompts to unpublish ALL versions', ->
        ...
        vpm.run(['unpublish', 'test-package'], callback)
    ...
    describe "when the package does not exist", ->
      it "calls back with an error", ->
        callback = jasmine.createSpy('callback')
        vpm.run(['unpublish', '--force', 'not-a-package'], callback)
  ...
  describe "when a version is specified", ->
    it 'unpublishes the version', ->
      callback = jasmine.createSpy('callback')
      vpm.run(['unpublish', '--force', 'test-package@1.0.0'], callback)
    ...
    describe "when --force is not specified", ->
      it 'prompts to unpublish that version', ->
        callback = jasmine.createSpy('callback')
        spyOn(Unpublish.prototype, 'prompt')
        vpm.run(['unpublish', 'test-package@1.0.0'], callback)
    ...
    describe "when the version does not exist", ->
      it "calls back with an error", ->
        callback = jasmine.createSpy('callback')
        vpm.run(['unpublish', '--force', 'test-package@2.0.0'], callback)
